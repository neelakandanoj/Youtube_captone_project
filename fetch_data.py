...
def get_channel_data(y_tube, channel_id):
    channel_info_request = y_tube.channels().list(
...
def get_video_ids(y_tube, playlist_id):
    video_ids = []
    next_page_token = None
    while True:
        try:
            video_id_request = y_tube.playlistItems().list(
                part='contentDetails',
...
def get_video_data(y_tube, video_id):
    videos_data = []
    for video_ids in video_id:
        video_data_request = y_tube.videos().list(
...
def get_comment_data(y_tube, video_id):
    comments_data = []
    for ids in video_id:
        try:
            video_data_request = y_tube.commentThreads().list(
                part="snippet",
                videoId=ids,
                maxResults=50
            ).execute()
